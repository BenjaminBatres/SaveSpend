...
export default function SidebarIcon({ Icon, label, link }) {
  ...
  return (
    <Link
      href={`${link}`}
      className={`relative group lg:w-full h-8 flex items-center hover:text-[#00bf91] transition-all duration-300 cursor-pointer ${
        pathname === link ? "lg:bg-gray-300 text-[#00bf91]" : ""
      }`}
    >
      <div
        className={`w-[3px] h-full mr-4 lg:w-0 lg:mr-0 ${
          pathname === link ? "bg-[#00bf91]" : "bg-transparent"
        }`}
      ></div>

...
export default function SubscriptionsBillsCard({ subscription, bill }) {
  return (
    <Link
    ...
    >
      <div className="flex justify-between items-center mb-2">
        ...
        >
          {subscription?.name?.[0] || bill?.name?.[0]}
        </div>
        <BsThreeDotsVertical className="text-gray-500" />
      </div>

      <div className="text-xl font-semibold text-[#2f4858] mb-1 truncate">
        {subscription?.name || bill?.name}
      </div>
      <div className="text-[#007787]">${subscription?.cost || bill?.cost}</div>
    </Link>
  );
}
